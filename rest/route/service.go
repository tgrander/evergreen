...
func AttachHandler(app *gimlet.APIApp, opts HandlerOpts) {
	...
	app.AddRoute("/admin/role_mapping").Version(2).Post().Wrap(superUser).RouteHandler(makeAddLDAPRoleMappingHandler(sc))
	app.AddRoute("/admin/role_mapping").Version(2).Delete().Wrap(superUser).RouteHandler(makeRemoveLDAPRoleMappingHandler(sc))
